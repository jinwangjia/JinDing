...
namespace SqlDal
{
    public class DeviceLogDal:ParentDal<DeviceLogDefinition>
    {
        ...
        public IList<DeviceLogDefinition> QueryByParam1(QueryByParam1In param)
        {
            param.BeginDateTime = param.BeginDateTime.Date;
            param.EndDateTime = param.EndDateTime.AddDays(1);
            using (var dal = new BaseDal())
            {
                var list = from o in dal.DeviceLog where ((o.UpdateDateTime >= param.BeginDateTime) && (o.UpdateDateTime < param.EndDateTime)) select o;
